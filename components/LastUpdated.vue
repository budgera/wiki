...
const { data: lastUpdate } = await useAsyncData(`last-update-git-${route.path}`, async () => {
  ...
  const owner = 'melishev'
  const repo = 'budgera-wiki'
...
<template>
  <div v-if="lastUpdate" class="space-y-1.5 text-sm text-neutral-600 dark:text-neutral-500">
    <p class="text-xs text-neutral-400 mb-0">Обновлено:</p>
